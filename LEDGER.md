...
| 0008 | 2026-09-24 | **SDR-1 — Substrate Determinism Receipt: PASS (A+B+C).** Preregistered at commit d9cc042 before execution (falsifiers committed first). Freeze window held: no code/playbook/schema changes between preregistration and this entry. **Phase A (cross-instance parity):** identical inputs (batch sdr1-parity, seed sdr1-determinism-777, 24 events, tenant SIMZ) fired on all four arms — H1 CONQUEST, H2 WAR, H3 FAMINE, H4 DEATH — returned identical checksum 0xc5f88dcf on 4/4 arms; per-index content sequences verified identical. **Phase B (replay determinism):** H3 FAMINE healed the same canonical batch (sdr1-replay, checksum 0x99f2ecca) twice — per-index decision vector (classification, matched playbook class, confidence, outcome) identical 24/24: 18 healed / 0 escalated / 6 detected-only at identical positions, 36 write-backs each, 0 scarcity deviations. **Phase C (temporal independence):** the two cycles ran at different wall-clock instants with different elapsed_ms (26,977 vs 33,493) and throughput under throttle (0.676 vs 0.540 heals/s) with identical outputs — wall-clock affected timing only, never a decision. **Verdict: the deterministic substrate replays identically — any instance, any time, any number of runs. No falsifier fired.** All inputs synthetic (SIMZ). No live tenants. No Lindy contact. |
